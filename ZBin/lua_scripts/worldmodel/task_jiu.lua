...
-- 接球
--可缺省参数 tar接球后朝向 dist 车球距离 needchase是否需要追球
...
function receiveBallV2(dist)---接球缓冲
	...
	local ipos = function(runner)
		...
		return ball.pos() + Utils.Polar2Vector((player.pos(runner) - ball.pos()):mod(),ball.vel():dir())
	end
	local idir = function(runner)
...
-- 		-- local turn = ball.velDir()>0 and 1 or -1
-- 		-- debugEngine:gui_debug_msg(CGeoPoint:new_local(-4500,ball.posY()-(ball.posX() +4500)*math.tan(ball.vel():dir())),0)

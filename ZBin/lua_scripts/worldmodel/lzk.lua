...
function ballspeed(role)
	debugEngine:gui_debug_msg(CGeoPoint:new_local(0,0),ball.velMod())
	debugEngine:gui_debug_msg(CGeoPoint:new_local(0,-300),ball.toPlayerDist(role))
end
function breakBycircle_lzk(angleLimit)--带球转身
	local ipos=function(runner)
	--debugEngine:gui_debug_msg(CGeoPoint:new_local(0,0),startAng)
	local res
		res=ball.pos()+Utils.Polar2Vector(50,ball.toPlayerDir(runner))
		return res
	end
	local idir=function(runner)
		local res
		if ball.toPlayerDist(runner)<90 then--and between(player.toBallDir(runner),player.dir(runner)-math.pi/36,player.dir(runner)+math.pi/36 )then
			if player.dir(runner)>angleLimit or player.dir(runner)<-angleLimit then
				res=player.dir(runner)
			-- elseif player.dir(runner)<0 then
			-- 	res=player.dir(runner)-math.pi/3
			else
				res=player.dir(runner)+math.pi*7/24
			end
		else
	...
	local mexe, mpos = GoCmuRush{pos = ipos, dir = idir, acc = iacc, flag = iflag,rec = r,vel = v}
	return {mexe, mpos}
 end
function breakdef(a,b)
	local p=CGeoPoint:new_local(4500,0)
	local ikpower=0
	local icpower=0
	local between = function(m,min,max)
	...
	local num = 0
	local GetEnemyballNum =function()
		local min = 9999
	    for j=0,param.maxPlayer-1 do 
	        if enemy.valid(j) then
				if (enemy.pos(j)-player.pos(a)):mod()<min  then
					min=(enemy.pos(j)-player.pos(a)):mod()
					num=j
				end
			end    
		end
	...
	local ipos=function(runner)
		local res
		local disttogoal=(player.pos(a)-p):mod()
		local ang1=(CGeoPoint:new_local(4500,300)-player.pos(a)):dir()
		local ang2=(CGeoPoint:new_local(4500,-300)-player.pos(a)):dir()
		local ang3=player.dir(a)
		local NUM= GetEnemyballNum()
		-- if disttogoal<1800 and between(ang3,ang2,ang1) then
		-- 	if task.CanPass(a,p) and ball.toPlayerDist(a)<120 then
		-- 		ipower=600
		-- 		res=ball.pos()+Utils.Polar2Vector(-95,player.toTheirGoalDir(runner))
		-- 	else
		-- 		ipower=0
		-- 		res=player.pos(runner)
		-- 	end
		local ToDist=(enemy.pos(NUM)-player.pos(runner)):mod()
		if not task.CanPass(a,b) and ToDist<400 then
			if (4500-math.abs(player.posY(a)))>400 then
				res=enemy.pos(NUM)+Utils.Polar2Vector(600,player.toPlayerDir(a,b)+math.pi/3)
			elseif player.posY(runner)>0 and (3000-math.abs(player.posY(runner)))<400 then
				res=enemy.pos(NUM)+Utils.Polar2Vector(600,player.toPlayerDir(a,b)-math.pi/3)
			else 
				res=enemy.pos(NUM)+Utils.Polar2Vector(600,player.toPlayerDir(a,b)+math.pi/3)
			end
		elseif task.CanPass(a,b) and ToDist<400 then
			res=ball.pos()+Utils.Polar2Vector(-95,player.toPlayerDir(runner,b))
			icpower=8000
			ikpower=3000
		else
			res=player.pos(runner)
	...
	local idir=function(runner)
		...
		local NUM= GetEnemyballNum()
		local ToDist=(enemy.pos(NUM)-player.pos(runner)):mod()
		-- if disttogoal<1800  then
		-- 	if task.CanPass(a,p) and ball.toPlayerDist(a)<120 then
		-- 		res=player.toTheirGoalDir(runner)
		-- 	else
		-- 		res=player.toPlayerDir(runner,b)
		-- 	end
		if not task.CanPass(a,b) and ToDist<400 then
			res=player.toBallDir(runner)
		elseif task.CanPass(a,b) and ToDist<400 then
	...
	local ikp=kp.specified(ikpower)
	local icp=cp.specified(icpower)
	local iflag=flag.dribbling
	local iacc=10000
	local mexe, mpos = GoCmuRush{pos = ipos, dir = idir, acc = iacc, flag = iflag,rec = r,vel = v}
	return {mexe, mpos,kick.chip,idir,pre.low,ikp,icp,flag.dribbling}
 end
-- function BigAttack(a,b,c)--a,b,c为三辆车a为runner
-- 	local iflag
-- 	local disttogoal=(player.pos(a)-goal):mod()
...
-- 	local ipos=function()
-- 		local res
-- 		if disttogoal<1500 then
-- 			if between(ang3,ang2,ang1) then
...
-- 				if ball.toPlayerDist(a)<=100 and ball.toPlayerDist(tar) >5000 then 
-- 					ipower=9*ball.toPlayerDist(tar)*0.12--模拟需乘8实地去掉
-- 				elseif  ball.toPlayerDist(a)<=110 and ball.toPlayerDist(tar) >3000 then
-- 					ipower=9*(ball.toPlayerDist(tar)*0.11+40)
-- 				else 
-- 					ipower=9*(ball.toPlayerDist(tar)*0.13+40)
-- 				end
-- 			else
-- 				-- if ball.toPlayerDist(a)<=100 and ball.toPointDist(tar) >5000 then 
-- 				-- 	ipower=ball.toPointDist(tar)*0.18
-- 				-- elseif ball.toPlayerDist(a)<=110 and ball.toPointDist(tar)>3000 then
-- 				-- 	ipower=(ball.toPointDist(tar)*0.12+40)
-- 				-- else 
-- 				-- 	ipower=(ball.toPointDist(tar)*0.14+40)
-- 				-- end
-- 				if ball.toPlayerDist(a)<=110 then
-- 				 	ipower=4000
-- 				else
-- 				 	ipower=0
-- 				end
...
-- 	local ikp= power()
-- 	local icp =cp.specified(CPpower)
	...
	local power=function()
		...
		if type(tar) == "string" then
			if ball.toPlayerDist(role1)<=100 and ball.toPlayerDist(tar) >5000 then 
				ipower=1*ball.toPlayerDist(tar)*0.12--模拟需乘8实地去掉
			elseif  ball.toPlayerDist(role1)<=110 and ball.toPlayerDist(tar) >3000 then
				ipower=1*(ball.toPlayerDist(tar)*0.11+40)
			else 
				ipower=1*(ball.toPlayerDist(tar)*0.13+40)
			end
		else
...
local mexe, mpos = GoCmuRush{pos = ipos, dir = idir, acc = a, flag = f,rec = r,vel = v}
	return {mexe, mpos,ikick,idir,pre.low,ikp,cp.specified(0),iflag}
end
function TouchShoot_lzk(tar,dist,needchase,role1,role2)--用于touch进攻已经 接球或者转身进攻
	local p=CGeoPoint:new_local(4500,0)
	local p1=CGeoPoint:new_local(4500,-400)
	local p2=CGeoPoint:new_local(4500,400)
	...
	local power=function()
		...
		return function ()
			...
			if between(Ang3,Ang2,Ang1) and ball.toPlayerDist(role1)<=600 then 
				ipower=700
	...
	local ipos = function(runner)
		...
		elseif needchase and ball.velMod()<3000 then --to getball early
			...
			if  math.abs(player.toPlayerDir(role2,role1))>=math.pi*4/12 then
				local ang1=math.abs((p-player.pos(runner)):dir())
				local ang2=math.abs(math.pi-ball.velDir())
				local ang3=math.abs(ang2-ang1)
				local R=75
				local Chang=R*math.sin(ang3)/math.sin(math.pi-ang2)
				...
				-- debugEngine:gui_debug_msg(CGeoPoint:new_local(1000,1000),Xm)
					res = CGeoPoint(ball.posX()-Chang,ball.posY()-R*math.sin(ang1)) + Utils.Polar2Vector(ball.toPlayerDist(runner),ball.vel():dir())
			else
					res = CGeoPoint(ball.pos()) + Utils.Polar2Vector(ball.toPlayerDist(runner),ball.vel():dir())
			end
		...
		elseif needchase and ball.velMod()<3000 then --to getball early
			...
			if  math.abs(player.toPlayerDir(role2,role1))>=math.pi*4/12 then
				...
				local Dist=(player.pos(runner)-CGeoPoint(Xm,Ym)):mod()
				 debugEngine:gui_debug_msg(CGeoPoint:new_local(1000,-1000),Dist)
				-- debugEngine:gui_debug_msg(CGeoPoint:new_local(1000,1000),Xm)
					res = CGeoPoint(ball.posX()-Chang,ball.posY()-R*math.sin(ang1)-10) + Utils.Polar2Vector(ball.toPlayerDist(runner)-10,ball.vel():dir())
			else
					res = CGeoPoint(ball.pos()) + Utils.Polar2Vector(ball.toPlayerDist(runner),ball.vel():dir())
			end
...
function receivandShoot_lzk(tar,dist,needchase,role1,role2)
	local idist = dist or 95
	local power=function()
		local ipower = 0
		return function ()
			if ball.toPlayerDist(role1)<=98 and ball.toPlayerDist(role2) >5000 then 
				ipower=ball.toPlayerDist(role2)*0.1140
			elseif  ball.toPlayerDist(role1)<=98 and ball.toPlayerDist(role2) >3000 then
	...
	local idir = function(runner)
		...
		if tar ~=nil then
				...
				elseif type(tar) == "function" then
					...
					if ball.toPlayerDist(runner)<=96 then
			...
			res = (player.pos(runner) - ball.pos()):dir()
		end
		return res
	end
	local ikp=power()
	local mexe, mpos = GoCmuRush{pos = ipos, dir = idir, acc = a, flag = f,rec = r,vel = v}
	-- return {mexe, mpos}	
	return {mexe, mpos, kick.flat, idir, pre.high, ikp, cp.specified(0), flag.dribbling}
end
...
function Dist(tar1,tar2)
	local res
	if type(tar2) == "string"then 
		 res=player.toPlayerDist(tar1,tar2)
	elseif type(tar2) == "function" then
		 res=(tar2()-player.pos(tar1)):dir()
	else
		res=(player.pos(tar1)-tar2):mod()
	end
	return res
end
function CanPass(role1,tar)--判断是否可以传球
	local angToOurplayer
	if type(tar) == "string" then 
		 angToOurplayer=player.toPlayerDir(role1,tar)
	elseif type(tar) == "function" then
		 angToOurplayer=(tar()-player.pos(role1)):dir()
	else
	...
	-- 	 angToOurplayer=player.toTheirGoalDir(role1)
		...
		for i=0,param.maxPlayer-1 do
			if enemy.valid(i) then
				...
				if angToOurplayer>angToenemy-ang-math.pi/7 and angToOurplayer<angToenemy+ang/7 then
					judge=0
					break
				end
			end

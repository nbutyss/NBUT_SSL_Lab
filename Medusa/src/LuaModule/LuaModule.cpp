...
extern "C" int Skill_canshoot(lua_State *L)
{
    TaskT playerTask;
    int runner = int(LuaModule::Instance()->GetNumberArgument(1, 0));
    playerTask.executor = runner;
    playerTask.player.carFront = LuaModule::Instance()->GetNumberArgument(2, 0);
    playerTask.player.lengthLimit = LuaModule::Instance()->GetNumberArgument(3, 0);
    playerTask.player.kickprecision = LuaModule::Instance()->GetNumberArgument(4, 0);
    playerTask.player.kickpower = LuaModule::Instance()->GetNumberArgument(5, 0);
//    playerTask.player.needkick = false;
    double x = LuaModule::Instance()->GetNumberArgument(6, 0);
    double y = LuaModule::Instance()->GetNumberArgument(7, 0);
    playerTask.player.flag = LuaModule::Instance()->GetNumberArgument(8, 0);
    playerTask.player.pos = CGeoPoint(x+100,y);
//    cout<<LuaModule::Instance()->GetNumberArgument(4, 0)<<endl;
    CPlayerTask* pTask = TaskFactoryV2::Instance()->Ccanshoot(playerTask);
    TaskMediator::Instance()->setPlayerTask(runner, pTask, 1);
    return 0;
}

extern "C" int nbut_pushNumber(lua_State* L)
{
    LuaModule::Instance()->PushNumber(WorldModel::Instance()->x);
    LuaModule::Instance()->PushNumber(WorldModel::Instance()->y);
    LuaModule::Instance()->PushNumber(WorldModel::Instance()->f[0]);
    LuaModule::Instance()->PushNumber(WorldModel::Instance()->f[1]);
    LuaModule::Instance()->PushNumber(WorldModel::Instance()->f[2]);
    LuaModule::Instance()->PushNumber(WorldModel::Instance()->best);
    LuaModule::Instance()->PushBool(WorldModel::Instance()->isCanshoot);
    return 7;
}
...
luaDef GUIGlue[] = 
{
    ...
    {"CCanShoot",            Skill_canshoot},

    {"Cnbut_pushNumber",    nbut_pushNumber},
	{NULL, NULL}
};

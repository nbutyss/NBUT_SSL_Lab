...
CCommandInterface::CCommandInterface(const COptionModule *pOption, QObject *parent)
    ...
    _thread = new std::thread([ = ] {
        GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-3000,1000),"receiveInformation",COLOR_YELLOW);
        receiveInformation();});
    _thread->detach();
}
...
void CCommandInterface::receiveInformation() {
    ...
    while (true) {
        ...
        while (receiveSocket->state() == QUdpSocket::BoundState && receiveSocket->hasPendingDatagrams()) {
            ...
            msg._mutex.lock();
            GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-4000,2000),"infrared : ",COLOR_YELLOW);
            msg.infrared = robot_status.infrared();
            if (msg.infrared){
                GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-3000,2000),"true",COLOR_YELLOW);
            }else{
                GDebugEngine::Instance()->gui_debug_msg(CGeoPoint(-3000,2000),"false",COLOR_YELLOW);
            }
